// Angled-corner button frame. `solid` fills it with the primary accent (color-1);
// otherwise it's a gradient outline that reads on both light and dark backgrounds.
const ButtonSvg = (solid?: boolean) => (
  <>
    <svg className="absolute top-0 left-0" width="21" height="44" viewBox="0 0 21 44">
      <path
        fill={solid ? "#AC6AFF" : "none"}
        stroke={solid ? "#AC6AFF" : "url(#btn-left)"}
        strokeWidth="2"
    ...
    >
      {solid ? (
        <polygon fill="#AC6AFF" fillRule="nonzero" points="100 0 100 44 0 44 0 0" />
      ) : (
        <>
    ...
    <svg className="absolute top-0 right-0" width="21" height="44" viewBox="0 0 21 44">
      <path
        fill={solid ? "#AC6AFF" : "none"}
        stroke={solid ? "#AC6AFF" : "url(#btn-right)"}
        strokeWidth="2"
