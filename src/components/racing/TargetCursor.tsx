...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const disabled = useMemo(() => {
    ...
    return reduced || coarse || (hasTouchScreen && isSmallScreen);
  }, []);
  ...
  const corner =
    "target-cursor-corner absolute top-1/2 left-1/2 h-3 w-3 border-[2.5px] border-color-1";
  return (
    <div
      ref={cursorRef}
    ...
    >
      <div
        ref={dotRef}
        className="absolute left-1/2 top-1/2 h-1 w-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-color-1"
        style={{ willChange: "transform" }}
      />
      <div
        className={`${corner} -translate-x-[150%] -translate-y-[150%] border-b-0 border-r-0`}
        style={{ willChange: "transform" }}
      />
      <div
        className={`${corner} translate-x-1/2 -translate-y-[150%] border-b-0 border-l-0`}
        style={{ willChange: "transform" }}
      />
      <div
        className={`${corner} translate-x-1/2 translate-y-1/2 border-l-0 border-t-0`}
        style={{ willChange: "transform" }}
      />
      <div
        className={`${corner} -translate-x-[150%] translate-y-1/2 border-r-0 border-t-0`}
        style={{ willChange: "transform" }}
      />
    </div>
  );
