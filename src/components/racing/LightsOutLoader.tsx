 ...
 * Five columns illuminate in sequence, hold, then cut to black-out and the
 * overlay lifts to reveal the site.
 *
 * Shown once per browser session. Skipped entirely for reduced-motion users.
 * Content always renders beneath it (this is a visual overlay, not a gate),
 * so LCP and SEO are unaffected.
 */
const COLUMNS = 5;
...
const LightsOutLoader = () => {
  ...
  useEffect(() => {
    // Read the real preference synchronously so we don't mark the loader done
    // before it resolves. Respect reduced motion and don't replay in a session.
    ...
    for (let i = 1; i <= COLUMNS; i++) {
      schedule(() => setLit(i), 200 + (i - 1) * 320);
    }
    // Hold all five, then lights out.
    schedule(() => setPhase("out"), 200 + COLUMNS * 320 + 520);
  ...
  return (
    <AnimatePresence>
      {phase !== "done" && (
        <motion.div
          key="lights-out"
          className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#070610]"
          initial={{ y: 0 }}
          animate={phase === "out" ? { y: "-100%" } : { y: 0 }}
          transition={
            phase === "out"
              ? { duration: 0.7, ease: [0.76, 0, 0.24, 1], delay: 0.35 }
              : { duration: 0 }
          }
          onAnimationComplete={() => {
            if (phase === "out") finish();
